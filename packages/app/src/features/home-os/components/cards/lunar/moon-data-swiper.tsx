...
function pagesFor(model: MoonCardModel, language: string): DataItem[][] {
...
export function MoonDataSwiper({ model, language }: { model: MoonCardModel; language: string }) {
  const rootRef = useRef<HTMLDivElement>(null);
  const swiperRef = useRef<Swiper | null>(null);
  const pages = pagesFor(model, language);
  ...
  return (
    <div
      ref={rootRef}
      className="swiper relative h-full min-h-0 w-full overflow-hidden [--swiper-pagination-bullet-inactive-color:currentColor] [--swiper-pagination-bullet-inactive-opacity:0.22] [--swiper-theme-color:currentColor] [&_.swiper-pagination-bullet]:!transition-all [&_.swiper-pagination-bullet-active]:!w-3 [&_.swiper-pagination-bullet-active]:!rounded-full"
